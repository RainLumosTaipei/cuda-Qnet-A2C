...
class A2C:
    ...
    def _test_env(self, vis=False):
        ...
        while not done:
            ...
            dist, _ = self.model.forward(state.tolist())
    ...
    def train(self):
        ...
        while frame_idx < self.__max_frames: 
            ...
            # 获取下一状态的价值估计
            next_y, next_value = self.model.forward(next_state.tolist())
            # 计算折扣累积回报
            ...
            self.model.backward(next_state.tolist(), next_y.logits.tolist(), advantage.tolist(), loss.tolist())
